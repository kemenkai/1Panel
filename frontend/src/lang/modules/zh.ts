...
const message = {
    ...
    toolbox: {
        ...
        windowsService: {
            title: '服务管理',
            helper: '在这里统一创建和维护主机服务。模板中的目录与文件名只是示例值，可以按实际安装目录修改。',
            template: '服务模板',
            templateHelper: '先选择与程序类型匹配的模板，再按实际部署路径调整下面的参数。',
            javaTemplate: 'Java 模板',
            dllTemplate: 'DLL 模板',
            packageTemplate: '交付包模板',
            nameHelper: '服务名称用于注册到系统服务管理器（Windows SCM / systemd），创建后不建议修改，建议使用稳定且易识别的英文名。',
            displayNameHelper: '显示名称用于系统服务管理器（Windows 服务管理器 / systemd）和面板列表展示，可填写更易读的中文或业务名称。',
            serviceType: '服务类型',
            serviceTypeHelper:
                'Java 会按 Java 进程方式启动；交付包填写模板目录时会自动判断 Docker Compose 是否可用，可用则按 Compose 服务启动，不可用则按目录内 JAR 展开为 Java 服务。',
            execPath: '启动程序路径',
            execPathHelperJava: 'Java / 交付包通常填写 java.exe 或完整的 Java 可执行文件路径，例如 C:\\Program Files\\Java\\bin\\java.exe。',
            execPathAutoHelper: 'Java 和交付包会自动使用 1Panel 安装目录下自带的 JDK，不需要手动填写 java.exe 路径。',
            execPathHelperDll: 'DLL 模板请填写实际启动程序路径，例如 service.exe。若程序依赖 DLL，可在下方 DLL 目录中补充运行时依赖。',
            jarPath: '程序包路径',
            jarPathHelper: 'JAR 包目录不固定，文件名也不固定，不要求必须叫 app.jar。只要填写可运行 JAR 的绝对路径即可。',
            jarPathHelperPackage: '交付包可上传单个 JAR，也可上传完整交付压缩包；目录和文件名都不固定。',
            jarUploadHelper: '直接上传可运行的 JAR 包即可。1Panel 会自动把它放到服务工作目录下，不需要手动填写 jarPath。',
            jarUploadHelperPackage:
                '支持上传单个 .jar，或上传完整 .zip/.tar.gz/.tgz/.tar.xz/.txz 交付包。压缩包会自动解压到 1Panel 服务目录，再按 Docker Compose 或 JAR 自动生成服务。',
            jarCurrentPath: '当前程序包：{0}',
            jarTargetPath: '上传后存放位置：{0}',
            jarRequired: '请选择可运行的 .jar 包；也支持 .zip、.tar.gz、.tgz、.tar.xz、.txz 格式的交付包。',
            jarOrTemplateRequired: '请上传交付包 .jar/.zip/.tar.gz/.tgz/.tar.xz/.txz 程序包，或填写交付包模板目录。',
            jarTemplateExclusive: '交付包的程序包和模板目录只能选择一种方式，请清空其中一项后再提交。',
            templateRefreshedAfterUpload: '已根据上传包刷新动态配置项，请确认参数后再次保存。',
            packagePreviewTitle: '自动识别服务',
            packagePreviewEmpty: '上传交付包或填写模板目录后，将自动识别并展示可创建的服务。',
            previewName: '服务名称',
            previewDisplayName: '显示名称',
            previewRuntime: '运行方式',
            previewSourcePath: '来源路径',
            runtimeJava: 'Java',
            runtimeDockerCompose: 'Docker Compose',
            createPreviewServices: '创建 {0} 个服务',
            workDir: '工作目录',
            packageTemplateDir: '交付包模板目录',
            packageTemplateDirHelper:
                '填写完整交付包模板目录，或直接上传 .zip/.tar.gz/.tgz/.tar.xz/.txz 交付包让 1Panel 自动解压。目录中如果有 docker-compose.yml、compose.yml 或 docker-compose-*.yml，并且当前机器可用 Docker Compose，会按 Compose 中的所有服务创建；如果没有 Docker Compose，会扫描目录下所有 .jar 并按 Java 服务创建。',
            workDirHelper: '这是程序运行时的工作目录。示例目录可改；如果留空，后端会回退到 execPath 所在目录。',
            workDirHelperDll: '这是本地程序启动时的工作目录，通常填写程序所在目录。示例目录仅供参考，不是固定要求。',
            workDirAutoHelper: '上传 JAR 后，1Panel 会自动在安装目录下创建该服务的工作目录，并使用它作为运行目录。',
            args: '启动参数',
            argsHelper: '可填写 JVM 参数和业务参数。若当前参数中没有 -jar，系统会自动补齐 -jar <jarPath>。',
            argsHelperDll: '填写传给本地程序的启动参数；如果不需要额外参数，可以留空。',
            dllDir: 'DLL 目录',
            dllDirHelper: '可选。用于补充程序依赖的 DLL 搜索目录，系统会在启动时追加到 PATH。',
            dllDirHelperPackage: '交付包如依赖本地 DLL，可在这里填写 DLL 目录；没有额外依赖时可留空。',
            configPath: '配置文件路径',
            configPathHelper: '可选。用于记录程序配置文件位置，便于后续运维查看；不填写也不会阻止服务创建。',
            configPathHelperPackage: '交付包通常填写 application-prod.yml 等配置文件绝对路径，方便统一记录和排查。',
            configPathAutoHelper: '启用内置配置模板后，1Panel 会自动在服务工作目录下生成 application-dev.yml，并自动把该路径写入启动参数。',
            envFilePath: '环境变量文件路径',
            envFilePathHelper: '可选。若留空，系统会在服务目录下自动生成默认 env 文件并写入 DLL_DIR、CONFIG_PATH、JAR_PATH。',
            configTemplateTitle: '内置配置模板',
            configTemplateSwitch: '启用内置配置模板',
            configTemplateSwitchHelper:
                '启用后无需上传 application-dev.yml。1Panel 会基于内置模板自动生成配置文件，并把下面填写的地址、端口和业务参数写入环境变量。',
            configTemplateSwitchHelperPackage:
                '交付包推荐直接启用内置配置模板。这样 JAR 上传完成后，配置文件、数据库地址和平台地址都会由 1Panel 自动生成。',
            configTemplateHelper:
                "模板来源于内置 application-dev.yml。1Panel 会自动识别其中的 {'${ENV:default}'} 动态占位符，并把下面填写的值写入服务 env 文件。",
            configTemplateSummary: '模板生成说明',
            configTemplateGeneratedPath: '生成配置文件：{0}',
            dynamicFieldTitle: '动态配置参数',
            dynamicFieldHelper: '配置项：{0}；默认值：{1}',
            dynamicFieldEmptyDefault: '空',
            dynamicFieldEmpty: "当前模板中未检测到 {'${ENV:default}'} 动态占位符，无需额外填写参数。",
            serverPort: '服务端口',
            serverPortHelper: '对应 server.port，默认使用 6084，可按目标环境修改。',
            applicationName: 'Spring 应用名',
            applicationNameHelper: '对应 spring.application.name，通常填写实际服务名或业务应用名。',
            redisHost: 'Redis 地址',
            redisHostHelper: '对应 spring.data.redis.host，仅需填写主机名或 IP，端口继续使用程序默认值。',
            platformBaseUrl: '平台基础地址',
            platformBaseUrlHelper: '对应 anzhener.crypto.platform.base-url，请填写完整的 http(s) 地址。',
            xxlJobAdminAddresses: 'XXL-Job 地址',
            xxlJobAdminAddressesHelper: '对应 xxl.job.admin.addresses，请填写完整地址，例如 http://127.0.0.1:9080/xxl-job-admin。',
            hisDbHost: 'HIS 数据库地址',
            hisDbHostHelper: '对应 HIS_DB_HOST，请填写 Oracle 数据库所在主机或 IP。',
            hisDbPort: 'HIS 数据库端口',
            hisDbPortHelper: '对应 HIS_DB_PORT，默认 1521。',
            hisDbServerName: 'HIS 服务名',
            hisDbServerNameHelper: '对应 HIS_DB_SERVER_NAME，例如 orcl。',
            hisDbSchema: 'HIS Schema',
            hisDbSchemaHelper: '对应 HIS_DB_SCHEMA，用于 Oracle schema 配置。',
            hisDbUser: 'HIS 数据库账号',
            hisDbUserHelper: '对应 HIS_DB_USER，请填写数据库连接用户名。',
            hisDbPassword: 'HIS 数据库密码',
            hisDbPasswordHelper: '对应 HIS_DB_PASSWORD，保存后会写入服务 env 文件。',
            orgCode: '医疗机构统一社会信用代码',
            orgCodeHelper: '对应 ANZHENER_ORG_CODE。',
            hospitalCode: '院区代码',
            hospitalCodeHelper: '对应 ANZHENER_HOSPITAL_CODE。',
            platformCode: '区域平台代码',
            platformCodeHelper: '对应 ANZHENER_PLATFORM_CODE，可为空，按业务接入要求填写。',
            winswPath: 'WinSW 路径',
            winswPathHelper: '填写 WinSW.exe 的来源路径。注册为 Windows 服务时，系统会复制该文件到服务目录并执行 install。',
            winswPathHelperOptional: '如果当前只生成服务文件而暂不注册，可以先保留此路径，后续开启“注册为 Windows 服务”时会使用它完成注册。',
            winswPathAutoHelper: 'Java 和交付包会自动使用 1Panel 安装目录下 tools/WinSW.exe，不需要手动填写此路径。',
            winswPathRequired: '开启“注册为 Windows 服务”时，必须填写有效的 WinSW.exe 路径。',
            registerService: '注册为系统服务',
            registerServiceHelper: '开启后会立即注册到系统服务管理器（Windows 用 WinSW，Linux 用 systemd）并纳入服务列表；关闭后只生成脚本、配置和 env 文件，不会注册服务。',
            autoStart: '开机自启',
            autoStartHelper: '控制服务的开机自动启动（Windows 为服务启动类型，Linux 对应 systemd enable）。开启后为自动启动，关闭后为手动启动。',
            autoStartHelperPending: '当前仅保存开机自启配置，等后续真正注册为系统服务时再按此设置应用。',
            argsHelperTemplate: '可填写 JVM 参数和业务参数。启用内置模板后，系统会自动补齐 -jar 和 --spring.config.location，无需手动再写配置文件路径。',
            servicePath: '服务文件路径',
            uninstallScriptPath: '卸载脚本路径',
            registrationStatus: '注册状态',
            registrationEnabled: '已注册',
            registrationDisabled: '仅生成文件',
            editConfig: '编辑配置',
            viewLog: '查看日志',
            configEditorTitle: '编辑配置',
            configEditorHelper: '这里只允许编辑当前服务绑定的配置文件和环境变量文件，保存后如需生效请手动重启服务。',
            configFileTab: '配置文件',
            envFileTab: '环境变量',
            filePath: '文件路径',
            saveAndRestartHint: '保存后不会自动重启服务，如需生效请手动重启。',
            noConfigFiles: '当前服务未登记可编辑的配置文件。',
            logTitle: '运行日志',
            logPath: '日志路径',
            logEmpty: '当前暂无日志文件，可能服务尚未启动。',
        },
        swap: {
    ...
    setting: {
        ...
        descriptionWindows: 'Windows 服务器运维管理面板',
        forum: '论坛求助',
        doc2: '使用手册',
