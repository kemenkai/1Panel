...
以一个包含多个业务服务的交付包为例，原包可以复用，但要按 1Panel 规则改造：

1. `service-a/service-b/service-c/service-d` 的 `docker-compose-*-web.yml` 只是给共享 Nginx 追加 volume，不是完整服务定义，不能直接作为 1Panel 应用。
...
5. `service-c` 原始 Nginx 片段里有明显路径错误：`service-b-ui` / `/service-b/index.html`，打包时应修正为 `service-c-ui` / `/service-c/index.html`。
...
- `package-service-a`
  独立包含 服务 A API + 服务 A Web。
- `package-service-b`
  独立包含 服务 B API + 服务 B Web。
- `package-service-c`
  独立包含 服务 C API + 服务 C Web。
- `package-service-d`
  独立包含 服务 D API + 服务 D Web。
...
```bash
./scripts/build_package_local_apps.sh \
  "./delivery-package/delivery-package" \
  "./build/package-local-apps"
```

生成后，把 `./build/package-local-apps/*` 复制到目标 1Panel 服务器的本地应用目录：

```bash
cp -R ./build/package-local-apps/* /opt/1panel/resource/apps/local/
```
...
./scripts/generate_1panel_java_app.sh \
  --spec ./docs/examples/java-service.spec.yaml
```
...
如果是复杂业务服务，建议用 `spec.yaml` 模式，把参数标准化保存下来。
现在脚手架会自动生成 `startJava.sh`，不再依赖交付包原包中的脚本文件。
