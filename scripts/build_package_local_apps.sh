#!/usr/bin/env bash
#
# Converts a delivery package's raw source tree into a set of 1Panel-recognizable
# local app packages (base framework + business apps).
#
# Usage:
#   ./scripts/build_package_local_apps.sh [source_dir] [output_dir] [version]
#
#   source_dir  Path to the delivery package's root directory (the directory that
#               contains install.conf, base_framework/, pda/, drg/, ptrp/, medical/).
#               Defaults to ./delivery-package/delivery-package relative to the repo
#               root; pass your own package's directory when using this elsewhere.
#   output_dir  Where the generated 1Panel local app directories are written.
#               Defaults to ./build/package-local-apps.
#   version     Version string used for the app version directory.
#               Defaults to 1.0.0.2.
...
SOURCE_DIR="${1:-${REPO_DIR}/delivery-package/delivery-package}"
OUTPUT_DIR="${2:-${REPO_DIR}/build/package-local-apps}"
VERSION="${3:-1.0.0.2}"
...
prepare_base_framework() {
    local app_dir="${OUTPUT_DIR}/package-base-framework"
    ...
    write_root_data_yml \
        ...
        "安装交付包共享基础服务" \
        "安装交付包共享基础服务，包含 MySQL、Redis、Nacos、Gateway、UPMS、Auth、XXL-Job。" \
        "Install the shared delivery package base stack including MySQL, Redis, Nacos, Gateway, UPMS, Auth, and XXL-Job." \
        "2048"
    write_root_readme \
        "${app_dir}" \
        "交付包基础框架" \
...
prepare_business_app() {
    ...
    if [[ "${patch_ptrp}" == "true" ]]; then
        replace_line "SERVER_NAME" "package-ptrp-biz" "${version_dir}/config/${api_config}"
    fi
    ...
    location ^~/ptrp/api/ {
        proxy_pass http://package-ptrp-api:6081/api/;
...
EOF
    ...
    location ^~/pda/pda-api/ {
        proxy_pass http://package-pda-api:8080/;
        proxy_connect_timeout 60s;
        proxy_read_timeout 120s;
        proxy_send_timeout 120s;
        proxy_set_header X-Real-IP $remote_addr;
...
EOF
    ...
    write_root_readme "${app_dir}" "${display_name}" "先确认交付包基础框架已安装并处于运行状态，再安装当前业务应用。该应用会加入共享的 \`1panel-network\`。"
    copy_logo "${app_dir}"
...
prepare_business_app \
    ...
    "交付包 PDA" \
    "安装交付包 PDA 业务应用" \
    "安装交付包 PDA 业务应用，默认接入基础框架提供的 Redis、Nacos 与 Gateway。" \
    "Install the delivery package PDA business app. It connects to the base stack through shared Redis, Nacos, and Gateway services." \
    "pda.conf" \
    "package-pda-api" \
    "package-pda-api" \
    "8080" \
...
prepare_business_app \
    ...
    "交付包 DRG" \
    "安装交付包 DRG 业务应用" \
    "安装交付包 DRG 业务应用，默认连接基础框架的 MySQL、Redis 和 Nacos。" \
    "Install the delivery package DRG business app. It connects to the base MySQL, Redis, and Nacos services." \
    "drg-ingroup.conf" \
    "package-drg-api" \
    "package-drg-api" \
    "6060" \
...
prepare_business_app \
    ...
    "交付包 PTRP" \
    "安装交付包 PTRP 业务应用" \
    "安装交付包 PTRP 业务应用，默认连接基础框架的 Redis 和 Nacos。" \
    "Install the delivery package PTRP business app. It connects to the base Redis and Nacos services." \
    "ptrp.conf" \
    "package-ptrp-api" \
    "package-ptrp-api" \
    "6081" \
...
prepare_business_app \
    ...
    "交付包 Medical" \
    "安装交付包 Medical 业务应用" \
    "安装交付包 Medical 业务应用，默认连接基础框架的 Gateway、Redis 和 Nacos。" \
    "Install the delivery package Medical business app. It connects to the base Gateway, Redis, and Nacos services." \
    "medical.conf" \
    "package-medical-api" \
    "package-medical-api" \
    "6666" \
