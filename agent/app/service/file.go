...
func applyDecompressOwnership(srcPath, dstPath string) error {
	...
	// Ownership IDs come from the platform-tagged helper (syscall.Stat_t on
	// unix, unavailable on windows) so this file still cross-compiles for the
	// Windows agent; on windows ok is false and we skip the chown.
	uid, gid, ok := fileOwnerIDs(info)
	if !ok {
		return nil
	}
	if info.Mode()&os.ModeSymlink != 0 {
		return os.Lchown(dstPath, uid, gid)
	}
	return os.Chown(dstPath, uid, gid)
}
