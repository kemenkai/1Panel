...
const message = {
    ...
    toolbox: {
        ...
        windowsService: {
            ...
            helper: 'Create and manage host services here. Template paths and file names are examples only and can be adjusted to your real installation directory.',
            template: 'Service Template',
            templateHelper: 'Choose the template that matches your program type first, then adjust the fields below to the real deployment paths.',
            javaTemplate: 'Java Template',
            dllTemplate: 'DLL Template',
            packageTemplate: 'Delivery Package Template',
            nameHelper: 'The service name is used when registering into the system service manager (Windows SCM / systemd). Use a stable English identifier and avoid changing it after creation.',
            displayNameHelper: 'The display name is shown in the system service manager (Windows Service Manager / systemd) and the panel list. You can use a more readable business name here.',
            serviceType: 'Service Type',
            serviceTypeHelper:
                'Java starts as a Java process. When a delivery package template directory is provided, 1Panel uses Docker Compose if available, otherwise it expands all JAR files in the directory as Java services.',
            execPath: 'Executable Path',
            execPathHelperJava: 'For Java / Delivery Package, this is usually java.exe or the full path to the Java executable, for example C:\\Program Files\\Java\\bin\\java.exe.',
            execPathAutoHelper: 'Java and Delivery Package services automatically use the bundled JDK under the 1Panel installation directory. You no longer need to enter java.exe manually.',
            execPathHelperDll: 'For the DLL template, fill in the real launcher path such as service.exe. If the program depends on DLLs, also set the DLL directory below.',
            jarPath: 'Package Path',
            jarPathHelper: 'The JAR directory is not fixed and the file name does not need to be app.jar. Any absolute path to a runnable JAR is valid.',
            jarPathHelperPackage: 'A delivery package can use a single JAR or a full delivery archive. Directory and file names are not fixed.',
            jarUploadHelper: 'Upload the runnable JAR directly. 1Panel stores it under the service working directory automatically, so you do not need to fill jarPath manually.',
            jarUploadHelperPackage:
                'Upload a single .jar or a full .zip/.tar.gz/.tgz/.tar.xz/.txz delivery archive. Archives are extracted to the 1Panel service directory, then services are generated from Docker Compose or JAR files automatically.',
            jarCurrentPath: 'Current Package: {0}',
            jarTargetPath: 'Target storage path: {0}',
            jarRequired: 'Please select a runnable .jar package. Delivery packages also support .zip, .tar.gz, .tgz, .tar.xz, and .txz archives.',
            jarOrTemplateRequired: 'Please upload a delivery package .jar/.zip/.tar.gz/.tgz/.tar.xz/.txz file or fill in the delivery package template directory.',
            jarTemplateExclusive: 'Choose either a delivery package or a template directory, not both.',
            templateRefreshedAfterUpload: 'Dynamic config fields were refreshed from the uploaded package. Please review the parameters and save again.',
            packagePreviewTitle: 'Detected Services',
            packagePreviewEmpty: 'Upload a delivery package or fill in a template directory to detect and preview services automatically.',
            ...
            createPreviewServices: 'Create {0} services',
            workDir: 'Working Directory',
            packageTemplateDir: 'Delivery Package Template Directory',
            packageTemplateDirHelper:
                'Fill in the full delivery package template directory, or upload a .zip/.tar.gz/.tgz/.tar.xz/.txz delivery archive and let 1Panel extract it automatically. If the directory contains docker-compose.yml, compose.yml, or docker-compose-*.yml and Docker Compose is available, all Compose services will be created. Without Docker Compose, all .jar files under the directory are created as Java services.',
            workDirHelper: 'This is the working directory used when the program starts. The example path can be changed; if left empty, the backend falls back to the execPath directory.',
            workDirHelperDll: 'This is the working directory for the local executable. It is usually the program directory, and the example path is only a reference.',
            workDirAutoHelper: 'After upload, 1Panel automatically creates the service working directory under the installation directory and uses it as the runtime directory.',
            args: 'Arguments',
            argsHelper: 'Use this for JVM arguments and application arguments. If -jar is not already present, the system appends -jar <jarPath> automatically.',
            argsHelperDll: 'Arguments passed to the local executable. Leave it empty if no extra startup arguments are needed.',
            dllDir: 'DLL Directory',
            dllDirHelper: 'Optional. Adds the directory to PATH at startup so dependent DLLs can be found.',
            dllDirHelperPackage: 'Optional. Use this when the delivery package depends on local DLLs; leave it empty when no extra DLL directory is needed.',
            configPath: 'Config File Path',
            configPathHelper: 'Optional. Used to record where the program configuration file is located for later operations and troubleshooting.',
            configPathHelperPackage: 'For a delivery package, this is usually an absolute path such as application-prod.yml so the runtime configuration location is clearly recorded.',
            configPathAutoHelper:
                'When the built-in config template is enabled, 1Panel generates application-dev.yml under the service working directory automatically and appends the path to the startup arguments.',
            envFilePath: 'Environment File Path',
            envFilePathHelper: 'Optional. If empty, the system generates a default env file under the service directory and writes DLL_DIR, CONFIG_PATH, and JAR_PATH into it.',
            configTemplateTitle: 'Built-in Config Template',
            configTemplateSwitch: 'Use built-in config template',
            configTemplateSwitchHelper:
                'When enabled, you do not need to upload application-dev.yml manually. 1Panel generates the config file from the built-in template and writes the values below into the service env file.',
            configTemplateSwitchHelperPackage:
                'Delivery packages are recommended to use the built-in config template so the config file, database address, and platform address are generated automatically after the JAR upload.',
            configTemplateHelper:
                "The template comes from the built-in application-dev.yml. 1Panel automatically detects {'${ENV:default}'} placeholders and writes the values below into the service env file.",
            configTemplateSummary: 'Template generation',
            configTemplateGeneratedPath: 'Generated config file: {0}',
            dynamicFieldTitle: 'Dynamic Config Values',
            dynamicFieldHelper: 'Config path: {0}; default: {1}',
            dynamicFieldEmptyDefault: 'empty',
            dynamicFieldEmpty: "No {'${ENV:default}'} placeholders were detected in the current template.",
            serverPort: 'Service Port',
            serverPortHelper: 'Maps to server.port. The default is 6084 and can be adjusted for the target environment.',
            applicationName: 'Spring Application Name',
            applicationNameHelper: 'Maps to spring.application.name. Usually this is the service or business application name.',
            redisHost: 'Redis Host',
            redisHostHelper: 'Maps to spring.data.redis.host. Enter only the host name or IP and keep the application default port.',
            platformBaseUrl: 'Platform Base URL',
            platformBaseUrlHelper: 'Maps to anzhener.crypto.platform.base-url. Enter a full http(s) URL.',
            xxlJobAdminAddresses: 'XXL-Job URL',
            xxlJobAdminAddressesHelper:
                'Maps to xxl.job.admin.addresses. Enter the full URL, for example http://127.0.0.1:9080/xxl-job-admin.',
            hisDbHost: 'HIS Database Host',
            hisDbHostHelper: 'Maps to HIS_DB_HOST. Enter the Oracle database host or IP.',
            hisDbPort: 'HIS Database Port',
            hisDbPortHelper: 'Maps to HIS_DB_PORT. The default is 1521.',
            hisDbServerName: 'HIS Service Name',
            hisDbServerNameHelper: 'Maps to HIS_DB_SERVER_NAME, for example orcl.',
            hisDbSchema: 'HIS Schema',
            hisDbSchemaHelper: 'Maps to HIS_DB_SCHEMA for the Oracle schema setting.',
            hisDbUser: 'HIS Database User',
            hisDbUserHelper: 'Maps to HIS_DB_USER.',
            hisDbPassword: 'HIS Database Password',
            hisDbPasswordHelper: 'Maps to HIS_DB_PASSWORD and is written into the service env file after saving.',
            orgCode: 'Organization Code',
            orgCodeHelper: 'Maps to ANZHENER_ORG_CODE.',
            hospitalCode: 'Hospital Code',
            hospitalCodeHelper: 'Maps to ANZHENER_HOSPITAL_CODE.',
            platformCode: 'Platform Code',
            platformCodeHelper: 'Maps to ANZHENER_PLATFORM_CODE. Leave it empty only when the business integration allows it.',
            winswPath: 'WinSW Path',
            winswPathHelper: 'Path to the source WinSW.exe. When registering the Windows service, the system copies it into the service directory and runs install.',
            winswPathHelperOptional: 'If you only want to generate the service files for now, keep this path here and it will be used later when you enable Windows service registration.',
            winswPathAutoHelper: '1Panel uses the bundled WinSW.exe under the installation directory automatically. You do not need to fill this path for Java or delivery packages.',
            winswPathRequired: 'A valid WinSW.exe path is required when "Register as Windows Service" is enabled.',
            registerService: 'Register as System Service',
            registerServiceHelper: 'When enabled, the panel immediately registers the service into the system service manager (WinSW on Windows, systemd on Linux). When disabled, it only generates scripts, config, and env files.',
            autoStart: 'Auto Start on Boot',
            autoStartHelper: 'Controls whether the service starts automatically on boot (Windows service startup type, or systemd enable on Linux). Enabled means automatic startup, disabled means manual startup.',
            autoStartHelperPending: 'The auto-start preference is stored now and will be applied later when the service is actually registered.',
            argsHelperTemplate:
                'Use this for JVM and business arguments. When the built-in template is enabled, the system also appends -jar and --spring.config.location automatically, so you do not need to write the config file path yourself.',
            servicePath: 'Service File Path',
            uninstallScriptPath: 'Uninstall Script Path',
            registrationStatus: 'Registration',
            registrationEnabled: 'Registered',
            registrationDisabled: 'Files Only',
            editConfig: 'Edit Config',
            viewLog: 'View Log',
            configEditorTitle: 'Edit Config',
            configEditorHelper:
                'Only the config file and env file bound to the current service can be edited here. Restart the service manually if you need the changes to take effect.',
            configFileTab: 'Config File',
            envFileTab: 'Environment',
            filePath: 'File Path',
            saveAndRestartHint: 'Saving will not restart the service automatically. Restart it manually if needed.',
            noConfigFiles: 'No editable config files are registered for the current service.',
            logTitle: 'Runtime Log',
            logPath: 'Log Path',
            logEmpty: 'No log file is available yet. The service may not have started.',
        },
        swap: {
            swap: 'Swap Partition',
            swapHelper1:
        ...
        clam: {
            clam: 'Virus scan',
            cron: 'Scheduled scan',
            cronHelper: 'Business Edition supports scheduled scans',
            ...
            alertHelper: 'Business Edition supports scheduled scans and SMS alerts',
    ...
    setting: {
        ...
        proxyHelper1: 'Installation package downloads and App Store sync (Business Edition only)',
        proxyHelper2: 'System updates and update checks (Business Edition only)',
        proxyHelper3: 'System license verification and synchronization',
        proxyHelper4: 'Network requests such as Docker image pulls go through the proxy server (Business Edition only)',
        proxyHelper5: 'Unified download and sync for system script libraries (Business Edition)',
        proxyHelper6: 'Certificate requests (Business Edition)',
        ...
        descriptionWindows: 'Windows Server Panel',
        forum: 'Discussions',
        doc2: 'Documentation',
    ...
    license: {
        ...
        lostHelper:
            'The license has reached the retry limit. Click Sync manually to make sure Business Edition features continue to work.',
        exceptionalHelper:
            'License sync verification failed. Click Sync manually to make sure Business Edition features work properly. Detail: ',
        quickUpdate: 'Quick update',
        import: 'Import',
        power: 'Authorize',
        unbindHelper: 'All Business Edition related Settings will be cleaned after unbinding. Continue? ',
        importLicense: 'Import license',
        importHelper: 'Click or drag the license file here',
        levelUpPro: 'Upgrade to Business Edition',
        licenseSync: 'License Sync',
        knowMorePro: 'Learn About Business Edition',
        closeAlert: 'The current page can be closed in the panel settings',
        introduce: 'Feature Introduction',
        waf: 'Upgrade to Business Edition to unlock interception maps, logs, block records, geo-blocking, custom rules, custom block pages, and more.',
        tamper: 'Upgrade to Business Edition to protect websites from unauthorized changes or tampering.',
        tamperHelper: 'Operation failed, the file or folder has tamper protection enabled. Please check and try again!',
        setting: 'Upgrade to Business Edition to customize the panel logo, welcome message, and more.',
        monitor:
            'Upgrade to Business Edition to view real-time website status, visitor trends, traffic sources, request logs, and more.',
        alert: 'Upgrade to Business Edition to receive SMS alerts, review alert logs, and stay on top of key events.',
        node: 'Upgrade to Business Edition to manage multiple Linux servers in 1Panel.',
        nodeApp: 'Upgrade to Business Edition to upgrade multi-node app versions without switching nodes manually.',
        nodeDashboard:
            'Upgrade to Business Edition for centralized management of multi-node apps, websites, databases, and scheduled tasks.',
        fileExchange: 'Upgrade to Business Edition to transfer files quickly between servers.',
        app: 'Upgrade to Business Edition to view service details, anomaly monitoring, and more in the mobile app.',
        cluster: 'Upgrade to Business Edition to manage MySQL/Postgres/Redis primary-replica clusters.',
        vllm: 'Upgrade to Business Edition to manage vLLM services in 1Panel with centralized deployment, configuration, lifecycle operations, and task tracking.',
        'model-downloader':
            'Upgrade to Business Edition to search, download, and manage local model files from HuggingFace, ModelScope, or compatible mirrors with centralized model directories and download tasks.',
    },
    clean: {
        scan: 'Start scanning',
    ...
    xpack: {
        expiresTrialAlert:
            'Friendly reminder: Your Business Edition trial will expire in {0} days, and all Business Edition features will no longer be accessible. Please renew or upgrade to the full version in a timely manner.',
        expiresAlert:
            'Friendly reminder: Your Business Edition license will expire in {0} days, and all Business Edition features will no longer be accessible. Please renew promptly to ensure continued usage.',
        menu: 'Business Edition',
        ...
        node: {
            ...
            syncWithMaster:
                'After upgrading to Business Edition, all data will be synced by default. Sync policies can be manually adjusted in node management.',
        ...
        customApp: {
            ...
            selectNode: 'Select Node',
            selectNodeError: 'Select a node',
            licenseHelper: 'The Business Edition supports the custom application repository feature',
        ...
        alert: {
            ...
            licenseHelper: 'Business Edition supports additional alert channels',
            ...
            alertConfigProHelper: 'Business Edition also supports WeCom, DingTalk, Feishu, and SMS alerts.',
        },
        theme: {
