...
const (
	localAppImportOverwrite = "overwrite"
	localAppImportSkip      = "skip"
	localAppImportFail      = "fail"
)

func (a AppService) UploadLocalAppPackage(packagePath, taskID, strategy string) (string, []string, []string, error) {
	appNames, existingApps, err := a.PreviewLocalAppPackage(packagePath)
	if err != nil {
		return "", nil, nil, err
	}
	...
	importTask.AddSubTask(
		...
		func(t *task.Task) error {
			return a.importLocalAppPackageTask(t, packagePath, strategy)
		},
		nil,
	)
	importTask.AddSubTask(
	...
	return importTask.Task.ID, appNames, existingApps, nil
}

func (a AppService) PreviewLocalAppPackage(packagePath string) ([]string, []string, error) {
	appNames, err := inspectLocalAppPackage(packagePath)
	if err != nil {
		return nil, nil, err
	}
	return appNames, existingLocalApps(appNames), nil
}

func (a AppService) importLocalAppPackageTask(taskItem *task.Task, packagePath, strategy string) error {
	...
	if strategy == "" {
		strategy = localAppImportOverwrite
	}
	switch strategy {
	case localAppImportOverwrite, localAppImportSkip, localAppImportFail:
	default:
	...
	for _, appRoot := range appRoots {
		...
		if fileOp.Stat(targetDir) && strategy == localAppImportFail {
	...
	for _, appRoot := range appRoots {
		...
		if fileOp.Stat(targetDir) {
			if strategy == localAppImportSkip {
				taskItem.Log(fmt.Sprintf("skip existing local app [%s]", appName))
				continue
			}
...
func existingLocalApps(appNames []string) []string {
	fileOp := files.NewFileOp()
	existing := make([]string, 0)
	for _, appName := range appNames {
		if fileOp.Stat(path.Join(global.Dir.LocalAppResourceDir, appName)) {
